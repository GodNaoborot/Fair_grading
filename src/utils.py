...
# Пороги гейта в координатах ratio. При 0.1/0.9 гейт срабатывает только там,
# где f-функции и без него дают 0.997, и cond неотличима от nocond.
GATE_LO_DEFAULT = 0.3
GATE_HI_DEFAULT = 0.7
...
def build_model_cond(
    ...
    with pm.Model() as model:
        ...
        # Нормировать сигмоиды друг на друга нельзя: тогда p_5 + p_2 == 1
        # тождественно и f_2..f_5 выпадают из правдоподобия. Делим только
        # если сумма превысила 1 — при gate_lo < gate_hi этого не бывает.
